...
#+(and (or ALLEGRO CormanLisp LispWorks MCL) (not CL-HTTP))
...
(defMacro with-http-stream ((stream url)
  ...
  `(let* ((,url-sym (ensure-http-url ,url))
     ...
     (unwind-protect
       (progn
         (setf ,stream
               #+Allegro (socket::make-socket :remote-host (host-string ,url-sym) :remote-port (port ,url-sym) :format :binary)
               #+LispWorks (comm:open-tcp-stream (host-string ,url-sym) :element-type '(unsigned-byte 8))
               #+MCL (make-instance 'ccl::binary-tcp-stream :host (host-string ,url-sym) :port (or (port ,url-sym) 80) :element-type 'unsigned-byte)
	       #+CMU (sys:make-fd-stream (ext:connect-to-inet-socket (host-string ,url-sym) (port ,url-sym)) :buffering :full :element-type '(unsigned-byte 8))
	       #+sbcl (sb-sys:make-fd-stream (ext:connect-to-inet-socket (host-string ,url-sym) (port ,url-sym)) :buffering :full :element-type '(unsigned-byte 8))
	       )
         ...
         (map nil #'(lambda (char) (write-byte (char-code char) ,stream))
              ...
              (format nil "GET ~a HTTP/1.0~C~C~C~C"
                      (uri-namestring ,url-sym) #\Newline #\Linefeed #\Newline #\Linefeed))
         (force-output ,stream)
...
#+CL-HTTP
(defMacro with-http-stream ((stream url &key referer (accept '((:* :xml))) authorization)
                            &rest body
       ...
       (http:with-http-request
         (url :get
              ...
              (http::compute-standard-request-headers url
                                                      ...
                                                      :header-plist
                                                      ,(if referer
                                                        `(list :referer ,referer :accept ',accept)
                                                        `(list :accept ',accept))))
...
#-(or ALLEGRO CL-HTTP LispWorks MCL CMU sbcl)
