...
(eval-when (:compile-toplevel)
  (let* ((*class.atn* 'xqdm-atn)
    ...
    (bnfp:compile-atn-system #p"xml:xml-grammar.bnf"
                             ;; jaa.20030315 removed, but not tested. it worked so far non-compiled :compile #-CMU t #+CMU nil
			     :compile t
                             ;; don't load directly - leave that to the app
                             ;; :load t
			     ;; jaa 20030315 removed #+CMU :load t
                             :package *token-package*
                             :fasl-pathname "xml:code;atn-lib;xml-grammar.fasl" ; pod was .bin ; 2010-06-21 ufasl
...
(defMethod document-parser ((stream stream) &rest args
  ...
  (destructuring-bind (&key
    ...
    (with-parse-environment (stream :encoding encoding)
      ...
      ;;(format *trace-output* "~% stream = ~A encoding = ~A" stream encoding)			    
      ;;(format *trace-output* "~% args = ~A " args)
