...
(define-condition node-graph-condition (condition) ())

(define-condition node-graph-error (node-graph-condition xml-error) ())

(defMacro defNGC (name &optional super slots &rest options)
  `(defException (,name "NGC") ,(or super '(node-graph-error))
     ,slots
     ,@options))
...
(defNGC "Unique Root Element Required" ()
  ()
