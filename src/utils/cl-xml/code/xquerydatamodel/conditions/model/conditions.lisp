...
(defMacro defSC (name &optional super slots &rest options)
  `(defException (,name "SC") ,(or super '(schema-error))
     ,slots
     ,@options))
...
(defMacro defVC (name &optional super slots &rest options)
  `(defException (,name "VC") ,(or super '(validity-cerror))
     ,slots
     ,@options))


#+digitool
(dolist (sym '(defDMC defSC defWFC defWFCProduction defVC defSC defDMC))
...
(defNSC "Namespace Attribute Default" ()
  ((name :initarg :name :initform nil :reader condition-name)
   (default :initarg :default :initform nil :reader condition-default)
   (encoded :initarg :encoded :initform nil :reader condition-encoded))
  (:report-condition
...
(defNSC "Namespace Undetermined" (namespace-condition xml-warning)
  ((name :initarg :name :reader condition-name :initform nil))
...
(defNSC "PrefixedAttName Required" ()
  ((name :initform nil :initarg :name :reader condition-name))
  (:report-condition
   (lambda (condition stream)
     (with-accessors ((name condition-name)) condition
