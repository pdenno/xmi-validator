  ...
  (defun find-namespace (name &key (if-does-not-exist :error) (if-exists t)
    ...
    (if space
      (case if-exists
        ...
        ((nil) nil)
        (t
;;; POD "The consequences are undefined if new-name or any new-nickname conflicts with any existing package names."
;;; I want it to use sei model.lisp-package
         #+nil(when nicknames-p
           (rename-package space name nicknames))
         space))
      (case if-does-not-exist
        ((nil) nil)
        (:create (make-package name :use nil :nicknames nicknames))
        (:error
  ...
  (defun find-namespace (name &key (if-does-not-exist :error) (if-exists *namespace-mode*)
    ...
    (if space
      (ecase if-exists
        ...
        ((nil) nil)
        (:copy
         ;; reuse it directly. shadowing is disabled.
         space)
        (:use
         space))
      (case if-does-not-exist
...
(DEFNAMESPACE "http://www.w3.org/2001/XMLSchema-datatypes"
  (:INTERN)
...
(DEFNAMESPACE "http://www.w3.org/2001/XMLSchema"
...
(defNamespace "http://www.w3.org/2001/XMLSchema-instance"
  (:nicknames "xsi")
  (:export "type"))
