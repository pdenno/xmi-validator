...
(defparameter *essential-model-names* '(:PTYPES :OCL :MOFI :QVT :CMOF :PCMOF :UML211 :UML22 :UML23 :UML241 :uml25 :uml251
					:uml4sysml12
					:SYSML12 :SYSML13  :SYSML14 :SYSML16
					:QUDV :OPL :ODM :RDF-PROFILE :OWL-PROFILE :RDF-LIBRARY
					:XSD-LIBRARY :OWL-LIBRARY
					:UML-20131001-PTYPES
					:UML-PROFILE-L2-20090901 :UML-PROFILE-L2-20110701 :UML-PROFILE-20131001 ; not L2 any more
					:UML-PROFILE-20161101
...
(defmemo ref-items-memoized (model)
  (let ((ht (make-hash-table :test 'equal))
	(uri (href-uri model)))
    (loop for typ across (types model)
	  for ix = (format nil "~A#~A" uri (class-name typ))
	  do (setf (gethash ix ht) typ))
    (loop for pkg in (packages model)
	  for ix = (format nil "~A#~A" uri (name pkg))
	  do (setf (gethash ix ht) pkg))
    (setf (gethash "_0" ht) model)
    (when-bind (pretty (pretty-name model))
      (setf (gethash pretty ht) model))
    ht))
