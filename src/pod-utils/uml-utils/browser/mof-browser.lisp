...
(defun slot-def-documentation (slot)
  "Currently neither SBCL nor closer-mop implement the documentation method on slots,
   yet documentation is there. Thus this method."
  ;; ToDo: Find a way to muffle the $%#! warning that SBCL generates. Good luck!
  ;; Notice that just using (muffle-warning) will cause many other warning while compiling the file.
  (slot-value slot 'sb-pcl::%documentation))
...
(defmethod encode-for-url ((obj mofi:mm-type-mo))
  (macrolet ((esc (v) `(tbnl:url-encode (string ,v))))
    (format nil "type*~A*~A"
	    (esc
	     (when-bind (pkg (symbol-package (class-name obj)))
		(or (car (package-nicknames pkg))
		    (package-name (symbol-package (class-name obj))))))
	...
	(open-slots  (mapcar #'(lambda (x) (when x (my-slot-direct-slot x)))
			     (mapcar #'decode-for-url
				     (split
				      ...
				      #\|)))))
    ;; ToDo: Specialization at the mm level sure would be nice.
    (when class
...
(defun mof-slot-details (slot)
  ...
  (flet ((pr-dotted (ls) (format nil "~A.~A" (car ls) (cadr ls))))
    (with-html-output-to-string (stream) stream
     (htm
      (:p)
      (str (or (slot-def-documentation slot) ""))
      (when-bind (props
		  (append
