...
(defvar *class-context* nil "Set to an interned symbol naming the class that is the context of the parse.")
...
;;; ToDo: What is currently *class-context* probably ought to be a stack. See %%scope below.
;;;       I see that it is nested in lexical bounds, but a real stack would make debugging easier.
;;;       (This would be part of making every non-pure function in this application pure.)

;;; This is the toplevel grammar rule and is not called from within the grammar.
...
(defparse |ExpressionInOcl|
  ...
  (let ((*scope* scope))
    (cond ((and (eql tkn '|def|)     (not (attribute-p "def"     *class-context*)))    (parse |OclDef|))
	  ((and (eql tkn '|context|) (not (attribute-p "context" *class-context*)))    (parse |OclContextualizedExp|))
	  (T                                                                           (parse |OclExpression|)))))
...
(defparse |Factor| () ; ToDo: OclMessageExp NYI.
...
;;; ToDo: the grammar is a bit hosed here. iterate and forAll are different.
;;; This needs to be fixed before publishing the grammar.
;;; ModelPropertyCall := AttributeRef  | ( CollectionOperator CollectionOpBody ) | ( Operator OperatorArgs )
...
;;; I really think the iter-var has to be defined (and initialized).
...
;;; ToDo: It is not clear from the spec whether multiple or just two are allowed. I'm going with multiple.
;;; ForAllExp := 'forAll' '('  ( VariableId { ',' VariableId } '|' )? OclExpression ')'
;;; 2008-03-08: Added syntax forALL (p : TYPE | ...
(defparse |ForAllExp| (operand)
  ...
  (let ((bar-pos (found-at-pos #'(lambda (x) (eql x #\|)) stream))
    ...
    (cond ((null vars)
	  ...
	  (t ; multiple vars, Cartesian product. ToDo: Leave order 'backwards'?
...
(defparse |OtherCollectionOperator| ()
  (ecase (read-token stream)
    ...
    (|collectNested| 'ocl-collect-nested))) ; ToDo: not defined in spec, not in L3.

;;; (a) OtherCollectionOpBody ::= '(' (VariableDeclaration '|' )? OclExpression ')'
;;; (b) OtherCollectionOpBody ::= '('  arguments? ')'
;;; (c) OtherCollectionOpBody ::=  ('[' arguments ']')?   ; ToDo: NYI, is it for real?
...
(defparse |PathName| (symbol-only-p)
  ...
  (let ((type-name nil)
    ...
    ;; ToDo: currently not requiring the starting package to be a model-level one!
...
;;; ToDo: Enums in L3 are #something.
...
;;; CollectionLiteralPart ::= CollectionRange | OclExpression
;;; Holy $&%#! What a grammar!
(defparse |CollectionLiteralPart| ()
  (if (found-at-pos #'(lambda (x) (eql :dot-dot x)) stream) ; ToDo: seems wrong 2007-10-16
      (parse |CollectionRange|)
    (parse |OclExpression|)))

;;; 2012-02-22 (previously was returning (list lower upper)
;;; ToDo: CollectionRange not used in L3 (no :dot-dot)
...
;;; PrimitiveLiteralExp ::= IntegerLiteralExp | RealLiteralExp | StringLiteralExp | BooleanLiteralExp
...
;;; ToDo: 2012-02-24 Not sure why I can't use '*, but
...
;;; ToDo: This production is suspect.
