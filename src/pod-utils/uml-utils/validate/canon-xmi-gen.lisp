...
(defun xmi-ensure-xmi-root (doc)
  ...
  (if (xml-typep (xml-root doc) (xmi-sym "XMI"))
      (setf (xml-attributes (xml-root doc)) nil) 
      (let* ((whatever (xml-root doc))
	     (comment  (dom:create-comment doc (format nil " Generated by the NIST Validator on ~A." (now))))
	     (new-root (xml-make-node doc nil (xmi-sym "XMI") ; <======================================== 2019 maybe xml-add-elem?
				      (list comment whatever) :type 'xqdm:elem-node)))
	(setf (xml-attributes whatever)
	      (remove-if 
	       #'(lambda (x) 
		   (or (eql (dom:node-name x) (xmi-sym "version"))
		       #|pod18 (eql (dom:node-name x) '|http://www.w3.org/2001/XMLSchema-instance|::|schemaLocation|)|#))
	       (xml-attributes whatever)))
...
(defun xmi-fix-namespace-uri (doc)
  ...
  (loop for ns in (xqdm:namespaces (xml-root doc)) do
       (when-bind (model (find-model (xml-value ns) :error-p nil))
				...
				(and 
				 ...
				 (gethash (car refs?) xmiid2obj-ht)))
			      (loop for ref in refs?
				    for new-elem = (xml-add-elem node (dom:prefix attr) (dom:local-name attr)) do
			      ...
			      (let ((new-elem (xml-add-elem node (dom:prefix attr) (dom:local-name attr))))
				(xqdm:xml-set-content new-elem (car (xml-children attr)))
  ...
  (with-slots (xmiid2obj-ht) (processing-results mut)
    (let ((xmi-idref (xmi-sym "idref")))
      (depth-first-search 
       ...
       #'(lambda (node)
	   (when (dom:element-p node)
	     (when-bind (idref (find xmi-idref (xml-attributes node) :key #'xqdm:name))
...
(defun xmi-if-href-remove-type (doc)
  ...
  (let ((xmi-type (xmi-sym "type"))
    ...
    (depth-first-search 
     ...
     :do #'(lambda (node) 
	     (when (dom:element-p node)
	       (when (member href (xml-attributes node) :key #'xqdm:name)
		 (setf (xml-attributes node)
		       (remove-if #'(lambda (x) (eql xmi-type (dom:node-name x)))
				  (xml-attributes node)))))))))
