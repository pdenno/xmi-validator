...
(defmethod initialize-instance :after ((obj odm:|Triple|) &key)
  (when-debugging (:parser 5)
    (format t "~%*** Made Triple: ~A~%" obj))
  (push-last obj *triples*)) ; 2014-12-01 was push
...
(defparse |turtleDoc| ()
  (with-instance (|turtleDoc|) ; turtleDoc is defined in utils.lisp
     (loop until (eql :eof (peek-token stream)) do 
	  (setf *subject* nil)
	  (push (parse |statement|) |statement|))))
...
(defparse |statement| ()
  (let ((p (peek-token stream))
	...
	(setf result (parse |directive|))
	(setf result (parse |triples|)))
    (assert-token #\. stream)
    result))
...
(defparse |base-decl| ()
  ...
  (let ((iri (read-token stream)))
    ...
    (unless (typep iri 'iriref)
      (error 'turtle-parse-token-error :expect "an IRI" :got iri))
    (push-last (cons :base (iriref--name iri)) *prefix-defs*)))
...
;;; 2014-11-26 Check this one; it is comprehensible!
(turtle2rdf 
"
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
@prefix dc: <http://purl.org/dc/elements/1.1/> .
